...
@pytest.fixture
def dummy_measurement_decoder(nand_circuit, empty_landing_pads, timepoint, experiment_block):
    class DummyMeasurementDecoder(json.JSONDecoder):
        def __init__(self):
            ...
            self.__add_symbol(experiment_block)
    ...
    return DummyMeasurementDecoder

@pytest.fixture
def dummy_sample_decoder(nand_circuit, empty_landing_pads, timepoint):
    class DummySampleDecoder(json.JSONDecoder):
        ...
        def __add_symbol(self, obj):
            self.__symbol_table[obj.name] = obj
    return DummySampleDecoder


@pytest.fixture
def dummy_control_decoder(nand_circuit, empty_landing_pads, timepoint):
...
class TestControl:
    ...
    def test_control_subject_serialization(self, empty_landing_pads, dummy_control_decoder):
        c1 = Control(
            name='negative_gfp',
            sample=Sample(subject=empty_landing_pads)
        )
...
class TestSample:
    ...
    def test_sample_serialization(self, nand_circuit, timepoint, iptg, dummy_sample_decoder):
        s1 = Sample(
            subject=nand_circuit,
            treatments=[
                TreatmentReference.create_from(
...
class TestMeasurement:
    ...
    def test_measurement_serialization(self, nand_circuit, timepoint, iptg, empty_landing_pads, experiment_block, dummy_measurement_decoder):
        m1 = Measurement(
            type='FLOW',
            block=BlockReference(block=experiment_block),
            controls=[
